  ...
  template: `
    <form class="tesla-battery" [formGroup]="tesla">
      <h1>{{ title }}</h1>
        ...
        <app-tesla-stats [stats]="statss"></app-tesla-stats>
        ...
        <div class="tesla-controls cf" formGroupName="config">
          ...
          <div class="tesla-climate cf">
            ...
            <app-tesla-climate [limit]="tesla.get('config.temperature').value > 10" formControlName="climate"></app-tesla-climate>
...
export class TeslaBatteryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.statss = this.calculateStats(this.results, this.tesla.controls['config'].value);
    this.tesla.controls['config'].valueChanges.subscribe(data => {
      this.statss = this.calculateStats(this.results, data);
    });
